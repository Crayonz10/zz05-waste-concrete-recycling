...
We solve this game using backward induction. The detailed solution process is provided in Appendix \ref{app:prop1}. We now present the main result as follows.
...
In this subsection, we analyze the two-stage biform game where the manufacturer and recycler cooperate through investment cost-sharing. The biform game consists of two stages: (i) a cooperative stage where the parties determine investment cost-sharing and profit allocation via Shapley value; (ii) a non-cooperative stage where they compete in collection prices.

The detailed derivations are presented in Appendices \ref{app:prop2}--\ref{app:prop4}. We now summarize the main results.

%\subsubsection{Cooperative stage}

The cooperative game is constructed under any given competitive situation $(p_M, p_R)$. Following the standard approach in cooperative game theory, we compute the characteristic values $V_{p_M,p_R}(S)$ for all possible coalitions $S \in \{\emptyset, \{M\}, \{R\}, \{M, R\}\}$ using the maximin principle.

\begin{prop}
\label{prop:characteristic}
Under any competitive situation $(p_M, p_R)$, the characteristic functions of the cooperative game are given by:
\begin{enumerate}
\item For singleton coalition $\{M\}$:
\begin{equation}
V_{p_M,p_R}(\{M\}) = (s - p_M - w - \mu_M)(\alpha p_M - \beta p_R)
\end{equation}

\item For singleton coalition $\{R\}$:
\begin{equation}
V_{p_M,p_R}(\{R\}) =
\begin{aligned}
...
\item For the grand coalition $\{M,R\}$:
\begin{equation}
V_{p_M,p_R}(\{M,R\}) =
\begin{aligned}
...
The cooperative game defined by the characteristic functions in Proposition \ref{prop:characteristic} is convex and satisfies superadditivity. Specifically, for any $(p_M, p_R)$, we have:
\begin{equation}
V_{p_M,p_R}(\{M,R\}) - V_{p_M,p_R}(\{M\}) - V_{p_M,p_R}(\{R\})
= \frac{\theta^2}{2c}(\alpha p_M - \beta p_R)\big[(\alpha p_M - \beta p_R) + 2(\alpha p_R - \beta p_M)\big] \geq 0
\end{equation}
Consequently, $V_{p_M,p_R}(\{M,R\}) \geq V_{p_M,p_R}(\{M\}) + V_{p_M,p_R}(\{R\})$, and the Shapley value allocation belongs to the core.
\end{prop}

Based on the convexity property established in Proposition \ref{prop:convexity}, we can compute the Shapley value profit allocations $\varphi_M$ and $\varphi_R$ for the manufacturer and recycler, respectively:
\begin{equation}
\varphi_M(p_M, p_R) = (s - p_M - w - \mu_M)(\alpha p_M - \beta p_R) + \frac{\theta^2(\alpha p_M - \beta p_R)\big[(\alpha p_M - \beta p_R) + 2(\alpha p_R - \beta p_M)\big]}{4c}
\end{equation}
\begin{equation}
\varphi_R(p_M, p_R) =
\begin{aligned}
& (s - p_R - m - \mu_R)(\alpha p_R - \beta p_M) + (w - m)(\alpha p_M - \beta p_R) \\
& + \frac{\theta^2 \big[ (\alpha p_R - \beta p_M)^2 + (\alpha p_M - \beta p_R + \alpha p_R - \beta p_M)^2 \big]}{4c}
\end{aligned}
\end{equation}

%\subsubsection{Non-cooperative stage}

Using the Shapley value allocations $\varphi_M$ and $\varphi_R$ as payoff functions, we now analyze the non-cooperative price competition stage.

\begin{prop}
\label{prop:biform}
Under the two-stage biform game model, the equilibrium strategies are given by:
\begin{enumerate}
\item The equilibrium collection prices $(p_M^{C*}, p_R^{C*})$ satisfy:
\begin{equation}
p_M^{C*} = \frac{
\displaystyle
\begin{aligned}
...
\item The optimal conversion rate is:
\begin{equation}
k^{C*} = \frac{\theta(\alpha - \beta)(p_M^{C*} + p_R^{C*})}{c}
\end{equation}

\item The optimal cost-sharing proportion is:
\begin{equation}
...
\item The equilibrium Shapley value allocations are:
\begin{equation}
\varphi_M^{C*} = (s - p_M^{C*} - w - \mu_M)(\alpha p_M^{C*} - \beta p_R^{C*}) + \frac{\theta^2(\alpha p_M^{C*} - \beta p_R^{C*})\big[(\alpha p_M^{C*} - \beta p_R^{C*}) + 2(\alpha p_R^{C*} - \beta p_M^{C*})\big]}{4c}
\end{equation}
\begin{equation}
\varphi_R^{C*} =
\begin{aligned}
& (s - p_R^{C*} - m - \mu_R)(\alpha p_R^{C*} - \beta p_M^{C*}) + (w - m)(\alpha p_M^{C*} - \beta p_R^{C*}) \\
& + \frac{\theta^2 \big[ (\alpha p_R^{C*} - \beta p_M^{C*})^2 + (\alpha p_M^{C*} - \beta p_R^{C*} + \alpha p_R^{C*} - \beta p_M^{C*})^2 \big]}{4c}
\end{aligned}
\end{equation}
\end{enumerate}
\end{prop}
