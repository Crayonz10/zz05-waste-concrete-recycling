...
Exploring the impact of mu_M (manufacturer's transportation cost) on equilibrium outcomes
"""
...
def main():
    ...
    # Range of mu_M values (x-axis) - manufacturer's transportation cost
    mu_M_values = np.linspace(5, 60, 100)
    ...
    mu_M_valid = mu_M_values[valid_mask]
    ...
    fig.suptitle('Fig. 7. Impact of Manufacturer Transportation Cost ($\\mu_M$) on Equilibrium Outcomes',
                 fontsize=12, fontweight='bold', y=0.98)

    # Plot 1: Collection Prices
    ax1 = axes[0, 0]
    ax1.plot(mu_M_valid, results['p_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax1.plot(mu_M_valid, results['p_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax1.set_xlabel('Manufacturer Transportation Cost ($\\mu_M$, yuan/m$^3$)')
    ...
    ax1.set_xlim([mu_M_valid[0], mu_M_valid[-1]])
    ax1.text(0.02, -0.12, 'Fig.7(a)', transform=ax1.transAxes, fontsize=10, fontweight='bold')

    # Plot 2: Collection Quantities
    ax2 = axes[0, 1]
    ax2.plot(mu_M_valid, results['q_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax2.plot(mu_M_valid, results['q_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax2.plot(mu_M_valid, results['q_total'], color='#6B7B8C', label='Total (Q)', linewidth=1.8, linestyle='--')
    ax2.set_xlabel('Manufacturer Transportation Cost ($\\mu_M$, yuan/m$^3$)')
    ...
    ax2.set_xlim([mu_M_valid[0], mu_M_valid[-1]])
    ax2.text(0.02, -0.12, 'Fig.7(b)', transform=ax2.transAxes, fontsize=10, fontweight='bold')

    # Plot 3: Cost-sharing Ratio (n*) and Conversion Rate (k*)
    ax3 = axes[1, 0]
    ax3.plot(mu_M_valid, results['n'], color='#6B7B8C', label='Cost-sharing Ratio ($n^*$)', linewidth=1.8)
    ax3.plot(mu_M_valid, results['k'], color='#8B7355', label='Conversion Rate ($k^*$)', linewidth=1.8)
    ax3.set_xlabel('Manufacturer Transportation Cost ($\\mu_M$, yuan/m$^3$)')
    ax3.set_ylabel('Value')
    ax3.legend(loc='best', frameon=True, fancybox=False, edgecolor='gray', framealpha=0.9)
    ax3.grid(True, alpha=0.3, linestyle='--')
    ax3.spines['top'].set_visible(False)
    ax3.spines['right'].set_visible(False)
    ax3.set_xlim([mu_M_valid[0], mu_M_valid[-1]])
    ax3.text(0.02, -0.12, 'Fig.7(c)', transform=ax3.transAxes, fontsize=10, fontweight='bold')

    # Plot 4: Profits
    ax4 = axes[1, 1]
    ax4.plot(mu_M_valid, results['phi_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax4.plot(mu_M_valid, results['phi_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax4.set_xlabel('Manufacturer Transportation Cost ($\\mu_M$, yuan/m$^3$)')
    ...
    ax4.set_xlim([mu_M_valid[0], mu_M_valid[-1]])
    ax4.text(0.02, -0.12, 'Fig.7(d)', transform=ax4.transAxes, fontsize=10, fontweight='bold')

    plt.tight_layout(rect=[0, 0, 1, 0.96])

    # Save figure
    fig.savefig('simulation/mu_M_impact.png', dpi=300, bbox_inches='tight', facecolor='white')
    fig.savefig('simulation/mu_M_impact.pdf', bbox_inches='tight', facecolor='white')
    print("Figures saved: mu_M_impact.png, mu_M_impact.pdf")

    # Print summary statistics
    print("\n=== Summary Statistics (mu_M) ===")
    print(f"mu_M range: {mu_M_valid[0]:.2f} to {mu_M_valid[-1]:.2f}")
    print(f"\nAt mu_M = {mu_M_valid[0]:.2f}:")
