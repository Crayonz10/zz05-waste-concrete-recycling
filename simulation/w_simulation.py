...
Exploring the impact of w (transfer price) on equilibrium outcomes
"""
...
def main():
    ...
    # Range of w values (x-axis) - transfer price
    w_values = np.linspace(20, 60, 100)
    ...
    fig.suptitle('Fig. 6. Impact of Transfer Price ($w$) on Equilibrium Outcomes',
                 fontsize=12, fontweight='bold', y=0.98)

    # Plot 1: Collection Prices
    ax1 = axes[0, 0]
    ax1.plot(w_valid, results['p_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax1.plot(w_valid, results['p_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax1.set_xlabel('Transfer Price ($w$, yuan/m$^3$)')
    ...
    ax1.set_xlim([w_valid[0], w_valid[-1]])
    ax1.text(0.02, -0.12, 'Fig.6(a)', transform=ax1.transAxes, fontsize=10, fontweight='bold')

    # Plot 2: Collection Quantities
    ax2 = axes[0, 1]
    ax2.plot(w_valid, results['q_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax2.plot(w_valid, results['q_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax2.plot(w_valid, results['q_total'], color='#6B7B8C', label='Total (Q)', linewidth=1.8, linestyle='--')
    ax2.set_xlabel('Transfer Price ($w$, yuan/m$^3$)')
    ...
    ax2.set_xlim([w_valid[0], w_valid[-1]])
    ax2.text(0.02, -0.12, 'Fig.6(b)', transform=ax2.transAxes, fontsize=10, fontweight='bold')

    # Plot 3: Cost-sharing Ratio (n*) and Conversion Rate (k*)
    ax3 = axes[1, 0]
    ax3.plot(w_valid, results['n'], color='#6B7B8C', label='Cost-sharing Ratio ($n^*$)', linewidth=1.8)
    ax3.plot(w_valid, results['k'], color='#8B7355', label='Conversion Rate ($k^*$)', linewidth=1.8)
    ax3.set_xlabel('Transfer Price ($w$, yuan/m$^3$)')
    ax3.set_ylabel('Value')
    ax3.legend(loc='best', frameon=True, fancybox=False, edgecolor='gray', framealpha=0.9)
    ax3.grid(True, alpha=0.3, linestyle='--')
    ax3.spines['top'].set_visible(False)
    ax3.spines['right'].set_visible(False)
    ax3.set_xlim([w_valid[0], w_valid[-1]])
    ax3.text(0.02, -0.12, 'Fig.6(c)', transform=ax3.transAxes, fontsize=10, fontweight='bold')

    # Plot 4: Profits
    ax4 = axes[1, 1]
    ax4.plot(w_valid, results['phi_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax4.plot(w_valid, results['phi_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax4.set_xlabel('Transfer Price ($w$, yuan/m$^3$)')
    ...
    ax4.set_xlim([w_valid[0], w_valid[-1]])
    ax4.text(0.02, -0.12, 'Fig.6(d)', transform=ax4.transAxes, fontsize=10, fontweight='bold')

    plt.tight_layout(rect=[0, 0, 1, 0.96])

    # Save figure
    fig.savefig('simulation/w_impact.png', dpi=300, bbox_inches='tight', facecolor='white')
    fig.savefig('simulation/w_impact.pdf', bbox_inches='tight', facecolor='white')
    print("Figures saved: w_impact.png, w_impact.pdf")

    # Print summary statistics
    print("\n=== Summary Statistics ===")
    print(f"w range: {w_valid[0]:.2f} to {w_valid[-1]:.2f}")
    print(f"\nAt w = {w_valid[0]:.2f}:")
