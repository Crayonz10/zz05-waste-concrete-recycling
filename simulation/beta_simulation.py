...
Exploring the impact of beta on equilibrium outcomes
"""
...
def main():
    ...
    # Range of beta values (x-axis) - must be < alpha for valid results
    ...
    for beta in beta_values:
        if beta >= alpha:  # Ensure beta < alpha for valid results
            continue
    ...
    # Filter beta values - ensure n* is non-negative for valid interpretation
    valid_mask = (beta_values < alpha) & (np.array(results['n']) >= 0)
    ...
    fig.suptitle('Fig. 2. Impact of Competition Intensity ($\\beta$) on Equilibrium Outcomes',
                 fontsize=12, fontweight='bold', y=0.98)

    # Plot 1: Collection Prices
    ax1 = axes[0, 0]
    ax1.plot(beta_valid, results['p_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax1.plot(beta_valid, results['p_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax1.set_xlabel('Competition Intensity ($\\beta$)')
    ...
    ax1.set_xlim([beta_valid[0], beta_valid[-1]])
    ax1.text(0.02, -0.12, 'Fig.2(a)', transform=ax1.transAxes, fontsize=10, fontweight='bold')

    # Plot 2: Collection Quantities
    ax2 = axes[0, 1]
    ax2.plot(beta_valid, results['q_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax2.plot(beta_valid, results['q_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax2.plot(beta_valid, results['q_total'], color='#6B7B8C', label='Total (Q)', linewidth=1.8, linestyle='--')
    ax2.set_xlabel('Competition Intensity ($\\beta$)')
    ...
    ax2.set_xlim([beta_valid[0], beta_valid[-1]])
    ax2.text(0.02, -0.12, 'Fig.2(b)', transform=ax2.transAxes, fontsize=10, fontweight='bold')

    # Plot 3: Cost-sharing Ratio (n*) and Conversion Rate (k*)
    ax3 = axes[1, 0]
    ax3.plot(beta_valid, results['n'], color='#6B7B8C', label='Cost-sharing Ratio ($n^*$)', linewidth=1.8)
    ax3.plot(beta_valid, results['k'], color='#8B7355', label='Conversion Rate ($k^*$)', linewidth=1.8)
    ax3.set_xlabel('Competition Intensity ($\\beta$)')
    ax3.set_ylabel('Value')
    ax3.legend(loc='best', frameon=True, fancybox=False, edgecolor='gray', framealpha=0.9)
    ax3.grid(True, alpha=0.3, linestyle='--')
    ax3.spines['top'].set_visible(False)
    ax3.spines['right'].set_visible(False)
    ax3.set_xlim([beta_valid[0], beta_valid[-1]])
    ax3.text(0.02, -0.12, 'Fig.2(c)', transform=ax3.transAxes, fontsize=10, fontweight='bold')

    # Plot 4: Profits
    ax4 = axes[1, 1]
    ax4.plot(beta_valid, results['phi_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax4.plot(beta_valid, results['phi_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax4.set_xlabel('Competition Intensity ($\\beta$)')
    ...
    ax4.set_xlim([beta_valid[0], beta_valid[-1]])
    ax4.text(0.02, -0.12, 'Fig.2(d)', transform=ax4.transAxes, fontsize=10, fontweight='bold')

    plt.tight_layout(rect=[0, 0, 1, 0.96])

    # Save figure
    fig.savefig('simulation/beta_impact.png', dpi=300, bbox_inches='tight', facecolor='white')
    fig.savefig('simulation/beta_impact.pdf', bbox_inches='tight', facecolor='white')
    print("Figures saved: beta_impact.png, beta_impact.pdf")

    # Print summary statistics
    print("\n=== Summary Statistics ===")
    print(f"Beta range: {beta_valid[0]:.3f} to {beta_valid[-1]:.3f}")
    print(f"\nAt beta = {beta_valid[0]:.3f}:")
