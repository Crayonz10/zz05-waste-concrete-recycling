...
Exploring the impact of theta on equilibrium outcomes
"""
...
def main():
    ...
    # Range of theta values (x-axis) - technology parameter
    theta_values = np.linspace(50, 300, 100)
    ...
    fig.suptitle('Fig. 4. Impact of Technology Parameter ($\\theta$) on Equilibrium Outcomes',
                 fontsize=12, fontweight='bold', y=0.98)

    # Plot 1: Collection Prices
    ax1 = axes[0, 0]
    ax1.plot(theta_valid, results['p_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax1.plot(theta_valid, results['p_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax1.set_xlabel('Technology Parameter ($\\theta$)')
    ...
    ax1.set_xlim([theta_valid[0], theta_valid[-1]])
    ax1.text(0.02, -0.12, 'Fig.4(a)', transform=ax1.transAxes, fontsize=10, fontweight='bold')

    # Plot 2: Collection Quantities
    ax2 = axes[0, 1]
    ax2.plot(theta_valid, results['q_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax2.plot(theta_valid, results['q_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax2.plot(theta_valid, results['q_total'], color='#6B7B8C', label='Total (Q)', linewidth=1.8, linestyle='--')
    ax2.set_xlabel('Technology Parameter ($\\theta$)')
    ...
    ax2.set_xlim([theta_valid[0], theta_valid[-1]])
    ax2.text(0.02, -0.12, 'Fig.4(b)', transform=ax2.transAxes, fontsize=10, fontweight='bold')

    # Plot 3: Cost-sharing Ratio (n*) and Conversion Rate (k*)
    ax3 = axes[1, 0]
    ax3.plot(theta_valid, results['n'], color='#6B7B8C', label='Cost-sharing Ratio ($n^*$)', linewidth=1.8)
    ax3.plot(theta_valid, results['k'], color='#8B7355', label='Conversion Rate ($k^*$)', linewidth=1.8)
    ax3.set_xlabel('Technology Parameter ($\\theta$)')
    ax3.set_ylabel('Value')
    ax3.legend(loc='best', frameon=True, fancybox=False, edgecolor='gray', framealpha=0.9)
    ax3.grid(True, alpha=0.3, linestyle='--')
    ax3.spines['top'].set_visible(False)
    ax3.spines['right'].set_visible(False)
    ax3.set_xlim([theta_valid[0], theta_valid[-1]])
    ax3.text(0.02, -0.12, 'Fig.4(c)', transform=ax3.transAxes, fontsize=10, fontweight='bold')

    # Plot 4: Profits
    ax4 = axes[1, 1]
    ax4.plot(theta_valid, results['phi_M'], color=COLORS['M'], label='Manufacturer (M)', linewidth=1.8)
    ax4.plot(theta_valid, results['phi_R'], color=COLORS['R'], label='Recycler (R)', linewidth=1.8)
    ax4.set_xlabel('Technology Parameter ($\\theta$)')
    ...
    ax4.set_xlim([theta_valid[0], theta_valid[-1]])
    ax4.text(0.02, -0.12, 'Fig.4(d)', transform=ax4.transAxes, fontsize=10, fontweight='bold')

    plt.tight_layout(rect=[0, 0, 1, 0.96])

    # Save figure
    fig.savefig('simulation/theta_impact.png', dpi=300, bbox_inches='tight', facecolor='white')
    fig.savefig('simulation/theta_impact.pdf', bbox_inches='tight', facecolor='white')
    print("Figures saved: theta_impact.png, theta_impact.pdf")

    # Print summary statistics
    print("\n=== Summary Statistics ===")
    print(f"Theta range: {theta_valid[0]:.2f} to {theta_valid[-1]:.2f}")
    print(f"\nAt theta = {theta_valid[0]:.2f}:")
